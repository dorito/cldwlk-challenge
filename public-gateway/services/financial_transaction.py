...
from data.schemas import FinancialTransactionSchema, FinancialTransactionCreationSchema
...
class FinancialTransactionService:
  ...
  def create_financial_transaction(self, transaction_data: FinancialTransactionCreationSchema) -> FinancialTransactionSchema:
      try:
          ...
          transaction_data_as_dict['received_at'] = datetime.datetime.now().isoformat()
          transaction_data_as_dict['paid_at'] = transaction_data.paid_at.isoformat() if transaction_data.paid_at else None
          transaction_data_as_dict['due_at'] = transaction_data.due_at.isoformat() if transaction_data.due_at else None
          data_as_byte = f"{json.dumps(transaction_data_as_dict)}".encode("utf-8")
          self._kafka_producer.send(Config.KAFKA_FINANCIAL_TRANSACTION_TOPIC, data_as_byte)
  ...
  def list_financial_transactions(self) -> list[FinancialTransactionSchema]:
        try:
          ...
          if not profile_guid:
              raise Exception("Profile GUID not found")
          response = requests.get(f"{Config.CREDIT_MANAGER_FINANCIAL_TRANSACTION_LIST_ENDPOINT}{profile_guid}")
          response.raise_for_status()
          transactions = [FinancialTransactionSchema(**transaction) for transaction in response.json()]
          return transactions
        except Exception as e:
          LOGGER.error(e)
