    ...
    def create_loan(self, loan_data: CreditLoanCreationSchema) -> CreditLoanSchema:
        try:
            ...
            response = requests.post(
                Config.CREDIT_MANAGER_CREDIT_REQUEST_CREATE_ENDPOINT,
                data=data_as_json,
    ...
    def list_loans(self) -> list[CreditLoanSchema]:
        try:
            ...
            response = requests.get(
                f"{Config.CREDIT_MANAGER_CREDIT_REQUEST_LIST_ENDPOINT}{profile_guid}"
            )
            response.raise_for_status()
            credit_requests = [
                CreditLoanSchema(**credit_request) for credit_request in response.json()
            ]
